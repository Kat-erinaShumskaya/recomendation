...
class MainRecommender:
    ...
    def _get_similar_user(self, user_id, N):
        ...
        recs = self.model.similar_users(self.userid_to_id[user_id],
                                        N + 1)  # Юзер похож на себя -> рекомендуем N+1 юзера
    ...
    def _extend_with_top_popular(self, recommendations, N=5):
        ...
        if len(recommendations) < N:
            top_popular = [rec for rec in self.overall_top_purchases[:N] if
                           rec not in recommendations and rec != self.fake_id]
